...
def make_gauss_tree(mra, prec, a = 3.0, b = 0, o = [0.1, 0.2, 0.3]):
    a = 3.0
    if (b == 0):
        b = np.sqrt(a/np.pi)**3
...
class TestReMRChem:
    ...
    def test_spinor(self):
        ...
        La_comp.copy_fcns(real = make_gauss_tree(self.mra, self.prec))
        spinor_H.copy_components(La = La_comp)
        spinor_H.init_small_components(self.prec/10)
        spinor_H.normalize()
    ...
    def test_mul(self):
        ...
        spinorb1.init_small_components(self.prec/10)
        spinorb1.normalize()
        spinorb1 *= 2.0
    ...
    def test_normalize(self):
        ...
        comp1.copy_fcns(real = make_gauss_tree(self.mra, self.prec))
        comp2 = cf.complex_fcn()
        comp2.copy_fcns(real = make_gauss_tree(self.mra, self.prec))
    ...
    def test_density(self):
        ...
        dens = spinorb1.density(self.prec)
    ...
    def test_dot(self):
        comp1 = cf.complex_fcn()
        comp2 = cf.complex_fcn()
        comp1.copy_fcns(real = make_gauss_tree(self.mra, self.prec, a=1.3, b=100, o=[0.1, 0.2, 0.1]),
                        imag = make_gauss_tree(self.mra, self.prec, a=1.0, b=200, o=[0.1, 0.2, 0.2]))
        comp2.copy_fcns(real = make_gauss_tree(self.mra, self.prec, a=1.0, b=90),
                        imag = make_gauss_tree(self.mra, self.prec, a=1.0, b=200))

        spinorb1 = orb.orbital4c()
        spinorb1.copy_components(La = comp1, Lb=comp1)
        spinorb1.init_small_components(self.rec/10)

        spinorb2 = orb.orbital4c()
        spinorb2.copy_components(La = comp2, Lb = comp1)
        spinorb2.init_small_components(self.prec/10)

        rval, ival = spinorb1.dot(spinorb2)
        assert rval == pytest.approx(2.5307429558268524)
        assert ival == pytest.approx(-0.41650020718445097)

    def test_overlap_density(self):
        comp1 = cf.complex_fcn()
        comp2 = cf.complex_fcn()
        comp1.copy_fcns(real = make_gauss_tree(self.mra, self.prec, a=1.3, b=100, o=[0.1, 0.2, 0.1]),
                        imag = make_gauss_tree(self.mra, self.prec, a=1.0, b=200, o=[0.1, 0.2, 0.2]))
        ...
        overlap = spinorb1.overlap_density(spinorb2, self.prec)
    ...
    def test_alpha(self):
        ...
        spinorb1.copy_components(La = comp1, Lb=comp1)
        spinorb1.init_small_components(self.prec/10)
        alphaorb = spinorb1.alpha_vector(self.prec)
    ...
    def test_dirac_hamiltonian(self):
        comp1 = cf.complex_fcn()
        comp2 = cf.complex_fcn()
        comp1.copy_fcns(real = make_gauss_tree(self.mra, self.prec, a=1.3, b=100, o=[0.1, 0.2, 0.1]),
                        imag = make_gauss_tree(self.mra, self.prec, a=1.0, b=200, o=[0.1, 0.2, 0.2]))
        ...
        spinorb1.copy_components(La = comp1, Lb=comp1)
        spinorb1.init_small_components(self.prec/10)
        spinorb2 = orb.apply_dirac_hamiltonian(spinorb1, self.prec, shift = 0.0, der = 'ABGV')
    ...
    def test_helmholtz(self):
        ...
        spinorb2 = orb.apply_helmholtz(spinorb1, mu, self.prec)
    ...
    def test_potential(self):
        ...
        Z = 1
        origin = [0.0, 0.0, 0.0]
        Peps = vp.ScalingProjector(self.mra, self.prec/10)
        f = lambda x: nucpot.coulomb_HFYGB(x, origin, Z, self.prec)
        V_tree = Peps(f)

        spinorb2 = orb.apply_potential(1.0, V_tree, spinorb1, self.prec)
    ...
    def test_complex_potential(self):
        ...
        spinorb2 = orb.apply_complex_potential(1.0, potential, spinorb1, self.prec)
